...
class GdgChapterRepository(gdgApiService: GdgApiService) {
    ...
    private class SortedData private constructor(
        ...
        companion object {
             ...
             */
            suspend fun from(response: List<GdgResponse>, location: Location?): SortedData {
                return withContext(Dispatchers.Default) {
                    // a filter list sorted by the distance from the current location
                    val filters: MutableList<String> = ArrayList()
                    val chapters: MutableList<GdgChapter> = ArrayList()
                    // group the chapters by region so that filter queries don't require any work
                    val chaptersByRegion: MutableMap<String, List<GdgChapter>> = HashMap()
                    // use distinctBy which will maintain the input order - this will have the effect of making
                    response.distinctBy { it.order }.forEach(Consumer { resp ->
                        // this sorting is too expensive to do on the main thread, so do thread confinement here.
                        val sortedChapters: List<GdgChapter> =
                            resp.chapters.sortByDistanceFrom(location)
                        filters.add(resp.title)
                        chapters.addAll(sortedChapters)
                        chaptersByRegion[resp.title] = sortedChapters
                    })
